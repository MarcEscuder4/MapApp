...
export const fetchPOIs = async (lat: number, lon: number, type: 'restaurant' | 'supermarket') => {
  ...
  const query = `
    ...
    (
      node["amenity"="${type}"](around:2000,${lat},${lon}); // Buscar nodos de tipo "restaurant" o "supermarket" dentro de 2 km
      way["amenity"="${type}"](around:2000,${lat},${lon});  // Buscar caminos (way) de tipo "restaurant" o "supermarket" dentro de 2 km
      relation["amenity"="${type}"](around:2000,${lat},${lon});  // Buscar relaciones (relation) de tipo "restaurant" o "supermarket" dentro de 2 km
    );
  ...
  return response.data.elements.map((element: any) => {
    ...
    return {
      ...
      name: element.tags?.name || type, // Si el POI tiene un nombre, lo usa; de lo contrario, usa el tipo (restaurante o supermercado)
      lat, // Latitud del POI
